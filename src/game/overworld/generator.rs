...
impl OverworldGenerator {
    ...
    pub fn gen_structure_pos(&self, structure: &Structure, cluster_pos: I64Vec3) -> StructurePos {
        ...
        let octant = cluster_pos.map(|v| v.div_euclid(octant_size as i64));
    ...
    pub fn gen_world_pos(&self, cluster_pos: I64Vec3) -> StructurePos {
        let reg = self.main_registry.registry();
        let world_st = reg.get_structure(self.main_registry.structure_world()).unwrap();
        let mut st_pos = self.gen_structure_pos(world_st, cluster_pos);

        if cluster_pos.is_zero() {
            st_pos.present = true;
        }
        st_pos
    }
    ...
    pub fn gen_spawn_point(&self) -> I64Vec3 {
        ...
        world_st.gen_spawn_point(self, world_seed, cache).unwrap()
    }
    ...
    pub fn generate_cluster(&self, cluster: &mut Cluster, pos: I64Vec3) {
        ...
        world_st.gen_cluster(self, world_seed, pos, cluster, cache);
