...
impl OverworldStreamer {
    ...
    pub fn update2(&mut self) -> HashSet<ClusterPos> {
    ...
    pub fn update(&mut self) -> HashSet<ClusterPos> {
        ...
        let num_threads = default_queue().current_num_threads() as u32;

        // Cluster load time ~ 2ms. Each update must load clusters for no more than 8ms. 8/2=4 clusters per thread.
        let max_loading_clusters_in_progress = num_threads * 4;
        // Cluster mesh update time ~ 2ms. Each update load updates meshes for another 10ms. 8/2~=4 clusters per thread.
        let max_updating_clusters_in_progress = num_threads * 4;
        ...
        // TODO: return what clusters need mesh update
