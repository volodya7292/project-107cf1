...
lazy_static! {
    static ref POSITIONS_SIMPLE: [na::Vector3<f32>; 12] = [
        na::Vector3::new(0.0, 0.0, 0.5),
        na::Vector3::new(0.0, 0.5, 1.0),
        na::Vector3::new(0.0, 1.0, 0.5),
        na::Vector3::new(0.0, 0.5, 0.0),
        na::Vector3::new(1.0, 0.0, 0.5),
        na::Vector3::new(1.0, 0.5, 1.0),
        na::Vector3::new(1.0, 1.0, 0.5),
        na::Vector3::new(1.0, 0.5, 0.0),
        na::Vector3::new(0.5, 0.0, 0.0),
        na::Vector3::new(0.5, 0.0, 1.0),
        na::Vector3::new(0.5, 1.0, 1.0),
        na::Vector3::new(0.5, 1.0, 0.0),
    ];
}

pub fn triangulate_simple(values: [bool; 8]) -> SmallVec<[na::Vector3<f32>; 12]> {
    let mut cube_index = 0_usize;

    if values[0] {
        cube_index |= 0b00000001;
    }
    if values[1] {
        cube_index |= 0b00010000;
    }
    if values[2] {
        cube_index |= 0b00001000;
    }
    if values[3] {
        cube_index |= 0b10000000;
    }
    if values[4] {
        cube_index |= 0b00000010;
    }
    if values[5] {
        cube_index |= 0b00100000;
    }
    if values[6] {
        cube_index |= 0b00000100;
    }
    if values[7] {
        cube_index |= 0b01000000;
    }
    ...
    let mut output = SmallVec::<[na::Vector3<f32>; 12]>::new();

    for i in 0..5 {
        if TRI_TABLE[cube_index][i * 3] == 255 {
            break;
        }
        for j in 0..3 {
            output.push(POSITIONS_SIMPLE[TRI_TABLE[cube_index][i * 3 + j] as usize]);
        }
    }
