...
void main(uint3 DTid : SV_DispatchThreadID) {
    ...
    while (parent_id != -1) {
        ...
        Bounds bbl = nodes.Load(parent.child_a).bounds;
		Bounds bbr = nodes.Load(parent.child_b).bounds;

        Bounds parent_bounds;
		parent_bounds.p_min = min(bbl.p_min, bbr.p_min);
		parent_bounds.p_max = max(bbl.p_max, bbr.p_max);

		nodes.StoreWithOffset(parent_id, LBVHNode_bounds_offset, parent_bounds);
