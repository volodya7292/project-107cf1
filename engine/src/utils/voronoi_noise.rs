...
impl VoronoiNoise3D {
    ...
    pub fn sample(&self, p: DVec3) -> (DVec3, f64) {
        let r: DVec3 = p.map(|v| v.round());
        let ri: I64Vec3 = glm::convert_unchecked(r);

        let mut min_p = DVec3::default();
        let mut min_dist = f64::MAX;

        for x in (ri.x - 1)..=(ri.x + 1) {
            for y in (ri.y - 1)..=(ri.y + 1) {
                for z in (ri.z - 1)..=(ri.z + 1) {
                    let mut rng = self.white_noise.state().next(x).next(y).next(z).rng();
                    ...
                    let v = glm::convert::<_, DVec3>(glm::vec3(x, y, z)) + jitter * 0.39614353;
